...
- Tailwind support is delivered via `@chamfer/tailwind` (CSS bundle + optional preset/plugin); keep utilities in sync with token updates.
...
- Playgrounds (`apps/playground/html|react/*`) demonstrate raw HTML/CSS usage and Tailwind/React adaptations with theme toggles.
...
- [x] Define linting/tooling stack (ESLint, Stylelint, Prettier or equivalent).
- [ ] Establish automated accessibility testing (Pa11y, Axe) in CI.
- [x] Map Chamfer tokens into Tailwind preset/plugin and document Tailwind usage.
- [ ] Capture Tailwind playground patterns inside docs once Astro shell exists.
