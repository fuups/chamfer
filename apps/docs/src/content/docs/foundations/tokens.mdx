...
description: How Chamfer tokens are generated, layered, exported, and themed.
status: draft
updated: "2025-01-07"
---

Chamfer’s design tokens flow through three layers so theming stays predictable and overrides remain scoped. This page explains how the layers relate, which artifacts are published, and how to customize them safely.

## Overview

- **Primitives → semantic → component** layers keep design intent separate from implementation details.
- **Multiple outputs** (CSS, JSON, TypeScript) support both runtime theming and build-time tooling.
- **Overrides** happen at the semantic layer, ensuring components, Tailwind utilities, and behaviors stay in sync.
...
1. Review the [Typography](/foundations/typography) guide for font and line-height tokens.
2. Explore the [Button component](/components/button) to see semantic tokens applied in practice.
3. Configure [Tailwind integration](/foundations/quick-start#_5-tailwind-integration-optional) if your project relies on utility classes.
